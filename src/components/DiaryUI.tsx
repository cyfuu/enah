...
export const DiaryUI = ({ onClose }: { onClose: () => void }) => {
    ...
    useEffect(() => {
        const updateScale = () => {
            ...
            const widthScale = (window.innerWidth - 28) / 910;
            const heightScale = (window.innerHeight - 180) / 700;
            const tunedScale = Math.min(widthScale, heightScale) * 0.9;
            setUiScale(Math.max(0.48, Math.min(1, tunedScale)));
        };
